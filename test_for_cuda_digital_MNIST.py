...
class MNIST_Net(nn.Module):
    def __init__(self, lr=0.1):
        ...
        self.blocks = nn.ModuleList([self.conv1, self.relu1, self.pool1,
                                     ...
                                     self.Flatten,
                                     self.linear7, self.relu7, self.dropout1,
                                     self.linear8, self.relu8, self.dropout2,
                                     self.linear9])

    def forward(self, x, save_memory=False):
        if save_memory:
            LBLFunction.set_state(torch.cuda.get_rng_state())
            ...
            x = LBLSGD(self.blocks[0:10], self.blocks[10:13], x)
            x = LBLSGD(self.blocks[0:13], self.blocks[13:16], x)
            x = LBLSGD(self.blocks[0:16], self.blocks[16:], x)
            return x
        else:
            x = self.conv1(x)
            x = self.relu1(x)
